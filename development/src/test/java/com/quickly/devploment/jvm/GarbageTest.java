...
public class GarbageTest {
	...
	/*
		1 执行引擎
			通常有 解释执行 和 编译执行 两种选择。
		2 Java 虚拟机 以方法作为最基本的执行单元， 栈帧 是用于支持虚拟机进行方法调用和方法执行背后的数据结构。栈帧存储了方法的局部变量表，操作数栈，动态链接，以及方法返回地址 等信息。
			在活动线程中，只有位于栈顶的方法 才是运行的，称为当前栈帧
			局部变量表，用于存放方法参数和方法内部定义的局部变量。Java 虚拟机通过索引定位的方式使用局部变量表，如果访问的是 32 位的数据类型，那么索引n 就代表的是使用第n 个变量槽，如果是64位的 那么就是 n 个+ 第n +1 个变量槽。
	 */
	/*
		1 即时编译 技术
			判定热点代码 1 基于采样的热点探测 2 基于计数器的热点探测。一旦计数器达到阈值 那么就会触发即时编译。
	 */
