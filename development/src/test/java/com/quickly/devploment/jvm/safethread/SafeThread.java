...
public class SafeThread {
	...
	/*
	 1 Thread wait  ，是Object 的方法，当线程A 调用wait 之后，就会进入到 wait set 集合中，其实每一个Object 都有有一个 wait set 集合，里面存着等待的线程
	 	当调用 notify 之后，线程不一定会执行，因为可能抢不到 LOCK 锁，也不一定顺序是 FIFO 的，线程被唤醒后 需要重新获取锁。
	 	notify all 相当于把wait set 集合中的所有线程 全部唤醒。
	 */
}
