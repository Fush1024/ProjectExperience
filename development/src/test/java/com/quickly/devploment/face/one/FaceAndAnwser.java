...
public interface FaceAndAnwser {
	...
	/*
	4 一颗现代处理器，每秒大概可以执行多少条简单的MOV指令，有哪些主要的影响因素？
	...
	/*
	5 为何 ConcurrentHashMap 1.8 放弃了 分段锁？
		 其实一切的优化和升级都是围绕时间和空间来进行的，jdk1.8中放弃分段锁，改用cas、synchronize、volatile。分段锁，其实让锁的粒度变大了，本来每个链表上发生冲突的可能性很小，现在把几个链表聚合在一起，显然增加了获取锁的冲突，所以时间上消耗更多。同时，之前使用ReentrantLock需要每个segment都继承ReentrantLock，显然增加了内存的开销。升级之后，在时间和空间上都进行了一定的优化。
		 另外，cas是unsfale的一类方法，通过这些方法可以实现cpu级别的原子操作，保证数据多个线程之间数据的一致性。ReentrantLock是基于AQS框架来实现的，AQS又是基于CAS和队列来实现的。底层其实都是用的unsafe提供的方法。
		 unsafe除了提供cas系列操作之外，还提供了对外内存的操作。对堆外内存的操作，提高了内存操作的效率。这其实相当于零拷贝，在linux系统里面，进行内存操作，会有一个从内核态到用户态的内存拷贝，内核态到用户态的拷贝是比较耗时的，如果只是短暂的使用一下内存数据，其实没必要复制到堆内存中，可以直接操作对外内存.
