...
public class SumHash {
	...
	public void testThreeSum() {
		int[] sums = {1, 3, 2, -2, -5, 0, 4, 13};
		List<List<Integer>> lists = threeSum(sums);
		log.info("三数之和 {}", lists);
	}
	...
	public List<List<Integer>> threeSum(int[] nums) {
		...
		if (zeroSize >= 3)
			res.add(Arrays.asList(0, 0, 0));//输出 三个 0 的情况
		...
		for (int v : nums) {
			if (v >= minValue && v <= maxValue) {//只保留在[minValue,maxValue]区间内的元素
				if (map[v - minValue]++ == 0) {//计数加去重
					if (v > 0)
						poses[posSize++] = v;//poses数组存所有去重后的正值
					else if (v < 0)
						negs[negSize++] = v;//negs数组存所有去重后的负值
				}
			}
		}
		//正负数两数组排序
		...
		for (int i = negSize - 1; i >= 0; i--) {//负数数组从后往前遍历
			...
			for (int j = basej; j < posSize; j++) {//正数数组从大于等于平均值的元素开始遍历
				...
				if (cv >= nv && cv <= pv) {
					...
					break;//如果 cv 小于 nv了，表明这种情况会在后面寻找，为避免重复输出，跳出循环
			}
		}
		return res;
	}
